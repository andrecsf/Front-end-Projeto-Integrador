      ...
      return Promise.allSettled(
        STATIC_ASSETS.map((asset) =>
          cache.add(asset).catch((err) =>
            console.warn(`[SW] Não foi possível fazer cache de: ${asset}`, err)
          )
        )
      );
    }).then(() => {
...
/**
 * Cache First: serve do cache se disponível, senão busca na rede e armazena.
 */
async function cacheFirst(request) {
  ...
  try {
    ...
    console.warn('[SW] Recurso não encontrado nem em cache nem na rede:', request.url);
    // Fallback para a página de login se for uma navegação
    if (request.mode === 'navigate') {
      const fallback = await caches.match('../Login/index.html');
      if (fallback) return fallback;
    }
    return new Response('Você está offline e este recurso não está em cache.', {
      status: 503,
...
async function networkFirst(request) {
  try {
    ...
    console.warn('[SW] API inacessível, tentando cache:', request.url);
    const cached = await caches.match(request);
    if (cached) return cached;

    return new Response(
      JSON.stringify({ erro: 'Você está offline. Não foi possível buscar os dados.' }),
      {
        status: 503,
